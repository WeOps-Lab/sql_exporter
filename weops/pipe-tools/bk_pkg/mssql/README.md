...
| mssql_session_blocked_seconds_in_wait      | Mssql会话阻塞等待时长         | block_role, server_name, db, session_id, blocking_session_id, hostname, loginame, program_name | 阻塞角色, 服务器名, 数据库名称, 会话ID, 阻塞会话ID, 主机名, 登录名, 程序名 | s       | gauge    | 原始指标     |
